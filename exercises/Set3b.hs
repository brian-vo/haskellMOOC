...
buildList start count end =  repeat start count [end] 
                          where repeat item number list
                                                 | number == 0 = list
                                                 | otherwise = repeat item (number-1) (item : list )
...
sums i =  listSum i []
        where 
            indSum count  
                        | count >= 0 = count + indSum (count-1)
                        | otherwise = 0
            listSum count list 
                            | count == 0 = list
                            | otherwise = listSum (count - 1) (indSum count : list)
...
sumsOf [] = []
sumsOf [a] = [a]
sumsOf (one:two:xs) = one : sumsOf((one+two):xs)
...
map2 f [] _ = []
map2 f _ [] = []
map2 f (one:xs) (on:ys) = f one on : map2 f xs ys
