...
velocity (Distance a) (Time d) = Velocity (a / d)
...
data State = Start | Error | Finished | EggsAdded | FlourAdded | SugarAdded | FSAdded | Mixed
  deriving (Eq,Show)
...
step FlourAdded AddSugar = FSAdded
step SugarAdded AddFlour = FSAdded
step FSAdded Mix = Mixed
...
addDuplicates :: Ord a => Set a -> Set a -> Set a
addDuplicates (Set a) (Set []) = Set a
addDuplicates (Set a) (Set (x:xs)) = addDuplicates (add x (Set a)) (Set xs)

instance Ord a => Semigroup (Set a) where
  Set a <> Set b = addDuplicates (Set a) (Set b)
...
passwordAllowed a (ContainsSome []) = False
passwordAllowed a (ContainsSome (x:xs))
                                      | x `elem` a = True
                                      | otherwise = passwordAllowed a (ContainsSome xs)
passwordAllowed a (DoesNotContain x) = not (passwordAllowed a (ContainsSome x))
passwordAllowed a (And req1 req2) = passwordAllowed a req1 && passwordAllowed a req2
passwordAllowed a (Or req1 req2) = passwordAllowed a req1 || passwordAllowed a req2
