...
maxBy :: (a -> Int) -> a -> a -> a
maxBy measure one two = if measure one > measure two then one else two
...
firstHalf string
          | even (length string) = take (div (length string) 2) string
          | otherwise = take (div (length string + 1) 2) string
...
joinToLength len xs = [moved | first <- xs, last <- xs, let moved = first ++ last, length moved == len]
...
sumRights (Right b:xs) = b + sumRights xs
...
multiCompose [] input = input
multiCompose xs input = multiCompose (init xs) ((last xs) input)
...
multiApp' :: [a->b] -> a -> [b] -> [b]
multiApp' [] _ cur = cur
multiApp' (s:xs) x cur = multiApp' xs x (s x : cur)
...
interpreter' (s:xs) x y list =
  case s of
    "up" -> interpreter' xs x (y + 1) list
    "down" -> interpreter' xs x (y - 1) list
    "left" -> interpreter' xs (x - 1) y list
    "right" -> interpreter' xs (x + 1) y list
    "printX" -> interpreter' xs x y (show x : list)
    "printY" -> interpreter' xs x y (show y : list)
